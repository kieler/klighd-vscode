...
const open = new Command('open')
    ...
    .action(async (file, options) => {
        ...
        const preferences = `${!resizeToFit ? '&resizeToFit=false' : ''}${clearData ? '&clearData=true' : ''}`
...
/** Returns an available port. A provided port overwrites the result regardless of whether or not it is available. */
async function findPort(port?: number): Promise<number> {
    return port ?? getPort({ port: [7000, 7001, 7002, 7003, 7004, 7005] })
}

/** Ensures that either a path or port is specified for the LS. Reads the options from the global options. */
function getLanguageServerAccess(global: typeof program): [path: string | undefined, port: number | undefined] {
