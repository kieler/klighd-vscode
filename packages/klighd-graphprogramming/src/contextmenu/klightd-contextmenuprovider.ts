...
export class ContextMenueProvider implements KlighdIContextMenuService{
    //idealy we would use the actionhandler to send messages
    //however since we can not know what actions / messages the server provides prior to reciving them 
    // here we have no registered action handler for the messages recived
    ...
    static enableMouseTargeting: boolean    // enables the drawing
    static startPos: Point|undefined        //defines the start position from which the line should show
    static selectSource: boolean            //determines if a source or target should be selected to reverse the edge 
    static destination: string | undefined; //when the mouse listener is done it will fill this field with the id of the selected element
    ...
    show(root: SModelRoot, anchor: Anchor, onHide?: (() => void) | undefined): void {
        ...
        if(!(selected[0].root.children[0] as any).properties['klighd.showContextmenu'] || 
            (selected[0].root.children[0] as any).properties['klighd.showContextmenu']==undefined)return
        ...
        if(selected.length == 1){
            ...
            for( const msg of structuredEditMesages){
                ...
                new_item.addEventListener('mousedown', (ev) => {
                    ...
                    if(hasSelect){
                        (async() => {
                            while(ContextMenueProvider.destination === undefined || ContextMenueProvider.enableMouseTargeting == true) // define the condition as you like
