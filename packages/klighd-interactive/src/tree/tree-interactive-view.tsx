...
export function renderHierarchyLevel(nodes: KNode[]): VNode {
    ...
    if (selectedNode) {
        ...
        let approxNodeSpacing = 999999

        if (direction === Direction.LEFT || direction === Direction.RIGHT) {
            selectedSiblings.sort((x, y) => getOriginalNodePositionY(x) - getOriginalNodePositionY(y))
            highlightedIndex = selectedSiblings.findIndex(
                (x) =>
                    getOriginalNodePositionY(x) + x.size.height / 2 >=
            ...
            highlightedIndex = selectedSiblings.findIndex(
                (x) =>
                    getOriginalNodePositionX(x) + x.size.width / 2 >=
        ...
        for (let i = 0; i < selectedSiblings.length - 1; i++) {
            ...
            if (direction === Direction.LEFT || direction === Direction.RIGHT)
                dist =
                    getOriginalNodePositionY(selectedSiblings[i + 1]) -
                    getOriginalNodePositionY(selectedSiblings[i]) -
                    selectedSiblings[i].size.height
            else
                dist =
                    getOriginalNodePositionX(selectedSiblings[i + 1]) -
                    getOriginalNodePositionX(selectedSiblings[i]) -
                    selectedSiblings[i].size.width

            if (approxNodeSpacing > dist) approxNodeSpacing = dist
        }

        if (selectedSiblings.length === 1) approxNodeSpacing = 0
        ...
        selectedSiblings.forEach((n) => {
            const x = getOriginalNodePositionX(n)
            const y = getOriginalNodePositionY(n)
            if (x < minX) {
                minX = x
            }
            if (y < minY) {
        ...
        for (let i = 0; i < selectedSiblings.length - 1; i++) {
            ...
            if (i === 0 && selectedSiblings[i].id !== selectedNode.id) {
                ...
                result = (
                    <g>
                        {result}
                        {renderCircle(i === highlightedIndex, x, y, color)}
                    </g>
                )
            }

            // Intermediate points
            if (selectedSiblings[i].id !== selectedNode.id && selectedSiblings[i + 1].id !== selectedNode.id)
                result = (
                    <g>
                        {result}
                        {renderCircle(i === highlightedIndex - 1, middleX, middleY, color)}
                    </g>
                )
            ...
            if (i === selectedSiblings.length - 2 && selectedSiblings[i + 1].id !== selectedNode.id) {
                if (direction === Direction.LEFT || direction === Direction.RIGHT) {
                    ...
                    y =
                        y2 +
                        selectedSiblings[selectedSiblings.length - 1].size.height +
                        approxNodeSpacing / 4 +
                        boundingBoxMargin / 2
                } else {
                    x =
                        x2 +
                        selectedSiblings[selectedSiblings.length - 1].size.width +
                        approxNodeSpacing / 4 +
                        boundingBoxMargin / 2
                    y = y2
                }
                ...
                result = (
                    <g>
                        {result}
                        {renderCircle(highlightedIndex === -1, x, y, color)}
                    </g>
                )
            }
        }
