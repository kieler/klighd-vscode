...
export function getLevels(nodes: KNode[]): KNode[][] {
    ...
    for (let i = 1; newNode; i++) {
        ...
        currentLevel = currentLevel
            .map((node) => getChildren(node))
            .reduce((node, otherNode) => node.concat(otherNode), [])
        levels[i] = []
...
function getLowestParent(nodes: KNode[], targetNode: KNode): KNode | undefined {
    ...
    const lowestParentPosition = Math.max(
        ...parents.map((node) =>
            node === undefined
                ? 0
                : dotProduct(
    ...
    const lowestParent = parents.find(
        (parent) =>
            (parent === undefined
                ? 0
                : dotProduct(
                      [parent.position.x + parent.size.width / 2, parent.position.y + parent.size.height / 2],
                      directionVector
                  )) === lowestParentPosition
    )
...
export function setTreeProperties(nodes: KNode[], event: MouseEvent, target: SModelElementImpl): Action {
    ...
    if (siblings.length === 0) return RefreshLayoutAction.create()
