...
import { getLayerOfNode, getLayers, getNodesOfLayer, HORIZONTAL_ARROW_X_OFFSET, HORIZONTAL_ARROW_Y_OFFSET, VERTICAL_ARROW_X_OFFSET, VERTICAL_ARROW_Y_OFFSET } from './constraint-utils';
...
function renderInLayerSuccessorOf(x: number, y: number, direction: Direction, color: string): VNode {
    ...
    return <g>
        {renderArrowInDirection(x + xOffset, y + yOffset, arrowDirection, color)}
    </g>
}
...
export function renderSetRelativeConstraint(node: KNode): VNode {
    ...
    if (inLayerPredecessorOfConstraint != null && inLayerSuccessorOfConstraint != null) {
        ...
        result = <g>{renderInLayerPredecessorOf(x + constraintOffset, y + 2 * constraintOffset, node.direction, "grey")}
                    {renderInLayerSuccessorOf(x + constraintOffset, y + constraintOffset, node.direction, "grey")}</g>
...
export function renderPositionIndicators(nodes: KNode[], selectedNode: KNode): VNode {
    const direction = selectedNode.direction
    const layers = getLayers(nodes, direction)
...
export function setForbiddenOnNodes(nodes: KNode[], selectedNode: KNode): void {
    ...
    for (const connectedNode of connectedNodes) {
        layerNodes = getNodesOfLayer(connectedNode.properties['org.eclipse.elk.layered.layering.layerId'] as number, nodes)
